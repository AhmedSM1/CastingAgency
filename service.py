...
def paginateActors(req, actors_list):
    page = request.args.get('page', 1, type=int)
    start = (page - 1) * DEFAULT_PAGE_SIZE
    end = start + DEFAULT_PAGE_SIZE
    all_actors = [actor.format() for actor in actors_list]
...
def paginateMovies(req, movies_list):
    page = request.args.get('page', 1, type=int)
    start = (page - 1) * DEFAULT_PAGE_SIZE
    end = start + DEFAULT_PAGE_SIZE
...
def getAllActorInMovie(movie_id):
    try:
        cast_list = Cast.query.filter_by(movie_id=movie_id).all()
        actors = []
...
def getAllMoviesFromActorId(actor_id):
    try:
        cast_list = Cast.query.filter_by(actor_id=actor_id).all()
        movies = []
        for cast in cast_list:
